...
public class AnswerRecordAdapterPresneterImpl implements IAnswerRecordAdapterPresenter {
    IAnswerRecordAdapterView iAnswerRecordAdapterView;
    Context context;
    User currUser;

    public AnswerRecordAdapterPresneterImpl(IAnswerRecordAdapterView iAnswerRecordAdapterView, Context context){
        this.iAnswerRecordAdapterView = iAnswerRecordAdapterView;
...
//                                answer.setAppend(false);
                                ...
                                iAnswerRecordAdapterView.showMessage("未知错误");
                            }

                        }catch (JSONException e){
//                            Log.d("JSONException：", e.getMessage());
                            iAnswerRecordAdapterView.showMessage("JSONException：" + e.getMessage());
                        }
                    }
                    @Override
                    public void onFailure(int statusCode, String error_msg) {
//                        Log.d("Get失败：", Integer.toString(statusCode));
                        iAnswerRecordAdapterView.showMessage(error_msg);
                    }
                });
    }
    ...
    void downloadAnswer(final Answer answer) {
        ...
        if (file.exists()) {
            Log.d("debug", "文件存在");
            iAnswerRecordAdapterView.play(answer);
            return;
        }
...
//                            iListenView.showMessage("Succsess");
                            ...
                            iAnswerRecordAdapterView.showMessage("FileNotFoundException：" + e.getMessage());
                        } catch (IOException e) {
                            iAnswerRecordAdapterView.showMessage("IOException：" + e.getMessage());
                        } finally {
                            // 关闭创建的流对象
