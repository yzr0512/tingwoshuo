...
import com.cqu.shixun.tingwoshuo.ui.AskRecordView.IAskRecordView;
...
public class AskRecordPresenterImpl implements IAskRecordPresenter {

    IAskRecordView iAskRecordView;
    public AskRecordPresenterImpl(IAskRecordView iAskRecordView){
        this.iAskRecordView = iAskRecordView;
    }

    @Override
    public void getAskRecordList(User user) {

        MyOkHttp myOkHttp = new MyOkHttp();
        myOkHttp.get().url("http://119.29.105.37:8000/myask")
...
//                                    question.setListenPrice(Float.valueOf(questionJson.get("listenPrice").toString())); // 暂时全部设置为1元
                                ...
                                iAskRecordView.showAskRecordList(questions);

                            }else{
                                String msg = response.getString("description");
                                iAskRecordView.showMessage(msg);
                            }

                        }catch (JSONException e){
//                            Log.d("JSONException：", e.getMessage());
                            iAskRecordView.showMessage("JSONException：" + e.getMessage());
                        }
                    }
                    @Override
                    public void onFailure(int statusCode, String error_msg) {
//                        Log.d("Get失败：", Integer.toString(statusCode));
                        iAskRecordView.showMessage(error_msg);
                    }
                });
    }
}
