...
public class AnswerRecordPresenterImpl implements IAnswerRecordPresenter {

    IAnswerRecordView iAnswerRecordView;

    public AnswerRecordPresenterImpl(IAnswerRecordView iAnswerRecordView){
        this.iAnswerRecordView = iAnswerRecordView;
    }

    @Override
    public void getAnswerRecordList(User user) {
        MyOkHttp myOkHttp = new MyOkHttp();
        myOkHttp.get().url("http://119.29.105.37:8000/myans")
...
//                                    question.setListenPrice(Float.valueOf(questionJson.get("listenPrice").toString())); // 暂时全部设置为1元
                                ...
                                iAnswerRecordView.showAnswerRecordList(questions);

                            }else{
                                String msg = response.getString("description");
                                iAnswerRecordView.showMessage(msg);
                            }

                        }catch (JSONException e){
//                            Log.d("JSONException：", e.getMessage());
                            iAnswerRecordView.showMessage("JSONException：" + e.getMessage());
                        }
                    }
                    @Override
                    public void onFailure(int statusCode, String error_msg) {
//                        Log.d("Get失败：", Integer.toString(statusCode));
                        iAnswerRecordView.showMessage(error_msg);
                    }
                });
    }
}
